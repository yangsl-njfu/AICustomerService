...
_node_path = os.path.join(_backend_dir, "services", "ai", "nodes", "turn_understanding_node.py")
_node_spec = importlib.util.spec_from_file_location(
    "backend.services.ai.nodes.turn_understanding_node",
    _node_path,
)
_node_mod = importlib.util.module_from_spec(_node_spec)
_node_mod.__package__ = "backend.services.ai.nodes"
sys.modules["backend.services.ai.nodes.turn_understanding_node"] = _node_mod
_node_spec.loader.exec_module(_node_mod)

TurnUnderstandingNode = _node_mod.TurnUnderstandingNode
