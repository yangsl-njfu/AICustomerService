...
职责:
1. 处理轻量寒暄，避免无意义调用模型。
2. 对业务范围内的问题执行附件优先、知识库补充的问答。
3. 对明显超出业务范围的问题，礼貌拒绝并引导回当前业务。
"""
...
class QANode(BaseNode):
    ...
    def _business_scope_hint(self, state: ConversationState) -> str:
        ...
        if active_intent:
            return f"{self._business_name(state)}相关内容，或刚才那条{active_intent}任务相关问题"

        return f"{self._business_name(state)}相关问题、资料解读、商品或服务说明、购买流程、订单与售后问题"

    def _scope_redirect_reply(self, state: ConversationState) -> str:
        business_name = self._business_name(state)
        scope_hint = self._business_scope_hint(state)
        return (
            f"这个话题我这边先不展开，我主要还是处理 {business_name} 相关问题。"
            f"您可以继续咨询{scope_hint}，我接着帮您处理。"
        )

    def _quick_reply_for_chitchat(self, message: str) -> Optional[str]:
        text = message.strip()
        if not text:
            return None
    ...
    def _is_light_chat(self, message: str, state: ConversationState) -> bool:
        if state.get("continue_previous_task"):
            return False
        text = message.strip()
        return bool(_LIGHT_CHAT_RE.match(text))

    def _business_profile(self) -> str:
    ...
    def _build_rag_prompt(self, state: ConversationState) -> ChatPromptTemplate:
...
{question}""",
    ...
    async def _prepare_messages(self, state: ConversationState):
        ...
        if self._is_light_chat(user_message, state):
        ...
        return self._build_rag_prompt(state).format_messages(
            business_name=self._business_name(state),
            scope_hint=self._business_scope_hint(state),
            business_profile=self._business_profile() or "当前未提供额外业务说明。",
            docs=docs_text,
    ...
    async def execute(self, state: ConversationState) -> ConversationState:
        ...
        state["response"] = content or self._scope_redirect_reply(state)
        return state
    ...
    async def execute_stream(self, state: ConversationState):
        ...
        if not full_response:
            full_response = self._scope_redirect_reply(state)
            yield full_response
